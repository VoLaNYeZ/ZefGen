...
export const urlsShareHost = (left, right) => {
    try {
        return new URL(String(left || '')).host === new URL(String(right || '')).host;
    } catch {
        return false;
    }
};
...
export const buildDirectAppstoreReviewWebhookUrl = ({ publicToken, supabaseUrl }) => {
    const normalizedSupabaseUrl = String(supabaseUrl || '').trim();
    if (!normalizedSupabaseUrl) return '';
    return withTokenQuery(`${normalizedSupabaseUrl}/functions/v1/appstore-review-webhook`, publicToken);
};
...
export const buildDefaultPublicWebhookUrl = ({ publicToken, publicSubdomain, publicBaseUrl, supabaseUrl, rootDomain }) => {
    ...
    if (normalizedPublicBaseUrl) return withTokenQuery(normalizedPublicBaseUrl, publicToken);
    return buildDirectAppstoreReviewWebhookUrl({ publicToken, supabaseUrl });
};

export const isUsingSharedDefaultWebhookUrl = ({ configuredUrl, defaultUrl }) => {
    const normalizedConfigured = String(configuredUrl || '').trim();
    const normalizedDefault = String(defaultUrl || '').trim();
    if (!normalizedDefault) return false;
    if (!normalizedConfigured) return true;
    try {
        return new URL(normalizedConfigured).host === new URL(normalizedDefault).host;
    } catch {
        return false;
    }
};
...
export const getWebhookStatusPayload = async ({ service, userId, appId }) => {
    ...
    const supabaseUrl = env('VITE_SUPABASE_URL', ['SUPABASE_URL']);
    const publicBaseUrl = env('APPSTORE_REVIEW_WEBHOOK_BASE_URL', ['VITE_APPSTORE_REVIEW_WEBHOOK_BASE_URL']);
    ...
    const resolvedWebhook =
        webhook && resolvedPublicSubdomain && resolvedPublicSubdomain !== String(webhook.public_subdomain || '').trim()
            ...
            : webhook;
    const defaultPublicWebhookUrl = webhook
        ? buildDefaultPublicWebhookUrl({
              publicToken: resolvedWebhook.public_token,
        ...
        : '';
    const internalListenerUrl = webhook
        ? buildDirectAppstoreReviewWebhookUrl({
              publicToken: resolvedWebhook.public_token,
              supabaseUrl,
          })
        : '';
    const explicitUrl = normalizePublicWebhookUrl(resolvedWebhook?.public_webhook_url);
    const isManagedExplicit = Boolean(extractManagedPublicSubdomainFromUrl(explicitUrl, rootDomain));
    const effectivePublicWebhookUrl = explicitUrl && !isManagedExplicit ? explicitUrl : defaultPublicWebhookUrl;
    const effectivePublicPageUrl = buildManagedPublicPageUrl({
        publicSubdomain: resolvedWebhook?.public_subdomain,
        rootDomain,
    });
    const pageReadinessIssues = [];
    if (
        !String(resolvedWebhook?.public_subdomain || '').trim() &&
        !String(namingHints?.appstoreName || '').trim()
    ) {
        pageReadinessIssues.push("Fill App's App Store name first.");
    }
    ...
    return {
        ...
        default_public_webhook_url: defaultPublicWebhookUrl,
        effective_public_webhook_url: effectivePublicWebhookUrl,
        effective_public_page_url: effectivePublicPageUrl,
        internal_listener_url: internalListenerUrl,
        using_shared_default_webhook_url:
            Boolean(defaultPublicWebhookUrl && internalListenerUrl) &&
            urlsShareHost(defaultPublicWebhookUrl, internalListenerUrl),
        ...
        page_readiness_issues: pageReadinessIssues,
    };
};
...
export const getPublicWebhookTarget = ({ webhook }) => {
    ...
    const defaultUrl = buildDefaultPublicWebhookUrl({
        ...
        publicSubdomain: resolvedPublicSubdomain,
        publicBaseUrl,
        supabaseUrl,
        rootDomain,
    });
    const internalUrl = buildDirectAppstoreReviewWebhookUrl({
        publicToken: webhook?.public_token,
        supabaseUrl,
    });
    const explicitUrl = normalizePublicWebhookUrl(webhook?.public_webhook_url);
    const isManagedExplicit = Boolean(extractManagedPublicSubdomainFromUrl(explicitUrl, rootDomain));
    const effectiveUrl = explicitUrl && !isManagedExplicit ? explicitUrl : defaultUrl;
    return {
        defaultUrl,
        internalUrl,
        effectiveUrl,
