...
export type GenerateScreenshotPromptsPayload = {
    clientSpec: string;
    appName?: string;
    appAlias?: string;
    slotCount: number;
...
export type GenerateScreenshotPromptsResponse =
    | {
          status: 'generated';
          appTheme: string;
          titles: string[];
...
const invokeApi = async (token: string, payload: GenerateScreenshotPromptsPayload) => {
    const response = await fetch('/api/generate-screenshot-prompts', {
        method: 'POST',
        headers: {
...
export const generateScreenshotPrompts = async (
    payload: GenerateScreenshotPromptsPayload
): Promise<GenerateScreenshotPromptsResponse> => {
    try {
        ...
        if (!response.ok) {
            ...
            if (isAuthRetryable(status, message)) {
                ...
                if (tokenExpMs && tokenExpMs - Date.now() > 30_000) {
                    return {
                        status: 'error',
                        error: 'Unauthorized by screenshot prompt API. Check Vercel VITE_SUPABASE_URL/VITE_SUPABASE_ANON_KEY.',
                    };
                }
                throw new Error(AUTH_EXPIRED_MESSAGE);
            }
        ...
        if (parsedStatus === 'generated') {
            return {
                status: 'generated',
                appTheme: String(body?.appTheme || ''),
                titles: Array.isArray(body?.titles)
                    ? body.titles.map((title: unknown) => String(title || '')).filter(Boolean)
                    : [],
        ...
        return {
            status: 'error',
            error: String(body?.error || 'Unexpected response from screenshot prompt API.'),
        };
    } catch (err: any) {
        ...
        return { status: 'error', error: message || 'Failed to generate screenshot prompts.' };
    }
};
