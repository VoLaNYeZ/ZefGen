...
import { generateScreenshotPrompts } from '../data/screenshot-prompt-autogen';
...
import {
    ...
    SCREENSHOT_PROMPT_AUTOGEN_MIN_SPEC_LENGTH,
...
type ScreenshotPromptAutogenParams = {
    clientSpec: string;
    isNoBrandMode: boolean;
    reportActionError: (message: string) => void;
    selectedApp: AppItem | null;
...
export function useNoBrandScreenshotPromptAutogenActions({
    clientSpec,
    isNoBrandMode,
    reportActionError,
    selectedApp,
    ...
    targetSlotCount,
    text,
}: ScreenshotPromptAutogenParams) {
    const [screenshotPromptAutogenBusy, setScreenshotPromptAutogenBusy] = useState(false);
    const canScreenshotPromptAutogen =
        Boolean(selectedApp) &&
        String(clientSpec || '').trim().length >= SCREENSHOT_PROMPT_AUTOGEN_MIN_SPEC_LENGTH;

    useEffect(() => {
        setScreenshotPromptAutogenBusy(false);
    }, [isNoBrandMode, selectedApp?.id, targetSlotCount]);

    const handleScreenshotPromptAutogen = useCallback(async () => {
        if (!session || !selectedApp) return;
        const normalizedClientSpec = String(clientSpec || '').trim();
        if (!normalizedClientSpec) {
            reportActionError(text('screenshot_prompt_autogen_need_spec'));
            return;
        }

        setScreenshotPromptAutogenBusy(true);
        try {
            const response = await generateScreenshotPrompts({
                clientSpec: normalizedClientSpec,
                appName: String(selectedApp.name || '').trim(),
                appAlias: String(selectedApp.alias || '').trim(),
            ...
            for (let slotIndex = 1; slotIndex <= targetSlotCount; slotIndex += 1) {
                const title = response.titles[slotIndex - 1] || '';
                setSlotPrompt(
                    slotIndex,
                    composeAutogeneratedScreenshotPrompt({
            ...
            showAliasNotice(text('screenshot_prompt_autogen_applied'));
        } catch (error: any) {
            reportActionError(String(error?.message || text('upload_failed')));
        } finally {
            setScreenshotPromptAutogenBusy(false);
