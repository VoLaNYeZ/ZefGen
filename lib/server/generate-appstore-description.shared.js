export const MIN_CLIENT_SPEC_LENGTH = 100;
export const MAX_DESCRIPTION_LENGTH = 4000;
export const MIN_DESCRIPTION_LENGTH = 1500;
export const APPSTORE_SUBTITLE_OPTION_COUNT = 5;
export const MAX_APPSTORE_SUBTITLE_LENGTH = 30;
export const APPSTORE_KEYWORDS_LENGTH = 100;
export const SPEC_MAX_CHARS = 8_000;
const MIN_KEYWORD_TOKEN_LENGTH = 3;
const MAX_KEYWORD_TOKEN_LENGTH = 18;
const SPEC_FALLBACK_CHARS = 4_000;

const SPEC_META_PATTERNS = [
    /\b\d+\+?\s*(templates?|variants?|tones?)\b/i,
    /\bchar(?:acter)?s?\s*(?:limit|count|range)\b/i,
    /\bjobs?-to-be-done\b/i,
    /\{(?:name|context|cta|signoff)\}/i,
    /\bplaceholder(?:\s+text)?\b/i,
    /^\s*(?:copy angle|cta|signoff|tone(?: options?)?|tones|template|templates|variant|variants|prompt notes?)\s*[:=-]/i,
    /^\s*(?:app store )?(?:description|copy)\s+(?:rules?|notes?|constraints?)\s*[:=-]/i,
];

const INTERNAL_LEAK_PATTERNS = [
    /\bjobs?-to-be-done\b/i,
    /\{(?:name|context|cta|signoff)\}/i,
    /\bplaceholder(?:\s+text)?\b/i,
    /\bfallback(?:\s+logic|\s+behavior)?\b/i,
    /\bchar(?:acter)?s?\s*(?:limit|count|range)\b/i,
    /\b\d+\+?\s*(templates?|variants?|tones?)\b/i,
    /\b(?:implementation|architecture|internal field)\b/i,
];

const BULLET_LINE_PATTERN = /^\s*[-*•]\s+\S+/;
const GENERIC_CTA_PATTERNS = [
    /\bdownload (?:the |this )?app(?:\s+today)?\b/i,
    /\bget started today\b/i,
    /\binstall now\b/i,
    /\btry (?:the )?app(?:\s+today)?\b/i,
    /\btap download\b/i,
    /\bstart (?:your|the) .* today\b/i,
];

const KEYWORD_STOPWORDS = new Set([
    'about',
    'after',
    'again',
    'also',
    'always',
    'another',
    'because',
    'before',
    'between',
    'could',
    'daily',
    'every',
    'first',
    'helps',
    'into',
    'just',
    'make',
    'more',
    'most',
    'much',
    'need',
    'only',
    'over',
    'people',
    'really',
    'still',
    'than',
    'that',
    'their',
    'them',
    'then',
    'there',
    'these',
    'they',
    'this',
    'through',
    'today',
    'very',
    'when',
    'with',
    'your',
]);

const KEYWORD_FALLBACK_TOKENS = [
    'focus',
    'clarity',
    'planner',
    'journal',
    'tracker',
    'routine',
    'wellness',
    'progress',
    'coach',
    'habits',
    'energy',
    'mindset',
    'calm',
    'notes',
    'goals',
];

const normalizeLine = (value) => String(value || '').trim();

const splitBlocks = (value) =>
    String(value || '')
        .split(/\n\s*\n+/)
        .map((block) => block.trim())
        .filter(Boolean);

const isLikelyMetaSpecLine = (line) => {
    const normalized = normalizeLine(line);
    if (!normalized) return false;
    if (SPEC_META_PATTERNS.some((rx) => rx.test(normalized))) return true;

    if (
        /^(?:notes?|constraints?|rules?)\s*:/i.test(normalized) &&
        /(?:char|placeholder|jobs-to-be-done|tone|variant|template|cta|signoff)/i.test(normalized)
    ) {
        return true;
    }

    return false;
};

export const sanitizeClientSpecForPrompt = (rawSpec) => {
    const source = String(rawSpec || '').replace(/\r/g, '').slice(0, SPEC_MAX_CHARS);
    const lines = source
        .split('\n')
        .map((line) => line.trim())
        .filter(Boolean)
        .filter((line) => !isLikelyMetaSpecLine(line));
    const sanitized = lines.join('\n').slice(0, SPEC_MAX_CHARS).trim();
    if (sanitized.length >= MIN_CLIENT_SPEC_LENGTH) return sanitized;
    return source.slice(0, SPEC_FALLBACK_CHARS).trim();
};

export const hasInternalLeakSignals = (text) => {
    const value = String(text || '');
    return INTERNAL_LEAK_PATTERNS.some((rx) => rx.test(value));
};

export const sanitizeDescription = (value) => {
    let out = String(value || '').trim();

    out = out.replace(/^```[a-zA-Z0-9_-]*\s*/i, '').replace(/\s*```$/i, '').trim();
    out = out
        .split(/\r?\n/)
        .map((line) => line.replace(/^\s{0,3}#{1,6}\s+/, ''))
        .join('\n')
        .trim();

    if (out.length > MAX_DESCRIPTION_LENGTH) {
        out = out.slice(0, MAX_DESCRIPTION_LENGTH).trimEnd();
    }

    return out;
};

export const sanitizeGeneratedSubtitleOption = (value) =>
    String(value || '')
        .replace(/^['"`“”]+/, '')
        .replace(/['"`“”]+$/, '')
        .replace(/\s+/g, ' ')
        .trim();

export const sanitizeGeneratedSubtitleOptions = (rawValue) => {
    const source = Array.isArray(rawValue) ? rawValue : [];
    const seen = new Set();
    const options = [];

    for (const item of source) {
        const normalized = sanitizeGeneratedSubtitleOption(item);
        if (!normalized) continue;
        const key = normalized.toLowerCase();
        if (seen.has(key)) continue;
        seen.add(key);
        options.push(normalized);
    }

    return options;
};

export const sanitizeGeneratedKeywords = (value) =>
    String(value || '')
        .replace(/^```[a-zA-Z0-9_-]*\s*/i, '')
        .replace(/\s*```$/i, '')
        .replace(/^['"`“”]+/, '')
        .replace(/['"`“”]+$/, '')
        .toLowerCase()
        .replace(/\s*,\s*/g, ',')
        .replace(/\s+/g, '')
        .replace(/[0-9]/g, '')
        .replace(/,+/g, ',')
        .replace(/^,|,$/g, '')
        .trim();

const isValidKeywordToken = (value) =>
    typeof value === 'string' &&
    /^[a-z]+$/.test(value) &&
    value.length >= MIN_KEYWORD_TOKEN_LENGTH &&
    value.length <= MAX_KEYWORD_TOKEN_LENGTH;

const dedupeKeywordTokens = (tokens) => {
    const seen = new Set();
    const out = [];

    for (const rawToken of tokens) {
        const token = String(rawToken || '').trim().toLowerCase();
        if (!isValidKeywordToken(token)) continue;
        if (seen.has(token)) continue;
        seen.add(token);
        out.push(token);
    }

    return out;
};

const tokenizeKeywordString = (value) => dedupeKeywordTokens(sanitizeGeneratedKeywords(value).split(',').filter(Boolean));

const extractKeywordCandidatesFromText = (value) => {
    const matches = String(value || '').toLowerCase().match(/[a-z]+/g) || [];
    return dedupeKeywordTokens(matches.filter((token) => !KEYWORD_STOPWORDS.has(token)));
};

const packKeywordTokensToLength = (tokens, targetLength = APPSTORE_KEYWORDS_LENGTH) => {
    const targetCost = targetLength + 1;
    const states = new Map([[0, []]]);

    for (const token of dedupeKeywordTokens(tokens)) {
        const cost = token.length + 1;
        const snapshot = Array.from(states.entries()).sort((a, b) => b[0] - a[0]);
        for (const [sum, selected] of snapshot) {
            const nextSum = sum + cost;
            if (nextSum > targetCost) continue;
            if (states.has(nextSum)) continue;
            states.set(nextSum, [...selected, token]);
        }
        if (states.has(targetCost)) break;
    }

    const exact = states.get(targetCost);
    return Array.isArray(exact) && exact.length > 0 ? exact.join(',') : '';
};

export const repairGeneratedKeywords = (rawValue, contextValues = []) => {
    const sanitized = sanitizeGeneratedKeywords(rawValue);
    const directAnalysis = analyzeGeneratedKeywords(sanitized);
    if (directAnalysis.issues.length === 0) {
        return directAnalysis.keywords;
    }

    const candidates = [
        ...tokenizeKeywordString(rawValue),
        ...contextValues.flatMap((value) => extractKeywordCandidatesFromText(value)),
        ...KEYWORD_FALLBACK_TOKENS,
    ];
    const packed = packKeywordTokensToLength(candidates);
    if (!packed) {
        return sanitized;
    }

    const packedAnalysis = analyzeGeneratedKeywords(packed);
    return packedAnalysis.issues.length === 0 ? packedAnalysis.keywords : sanitized;
};

export const analyzeGeneratedSubtitleOptions = (rawValue) => {
    const options = sanitizeGeneratedSubtitleOptions(rawValue);
    const issues = [];

    if (options.length < APPSTORE_SUBTITLE_OPTION_COUNT) {
        issues.push('subtitle_count');
    }

    if (options.some((option) => option.length > MAX_APPSTORE_SUBTITLE_LENGTH)) {
        issues.push('subtitle_length');
    }

    return {
        options: options.slice(0, APPSTORE_SUBTITLE_OPTION_COUNT),
        issues: Array.from(new Set(issues)),
    };
};

export const analyzeGeneratedKeywords = (rawValue) => {
    const keywords = sanitizeGeneratedKeywords(rawValue);
    const issues = [];

    if (!keywords) {
        issues.push('keywords_empty');
    }

    if (!/^[a-z,]+$/.test(keywords)) {
        issues.push('keywords_charset');
    }

    const keywordTokens = keywords.split(',').filter(Boolean);
    if (keywordTokens.length === 0 || keywordTokens.some((token) => !/^[a-z]+$/.test(token))) {
        issues.push('keywords_format');
    }

    if (keywords.length !== APPSTORE_KEYWORDS_LENGTH) {
        issues.push('keywords_length');
    }

    return {
        keywords,
        issues: Array.from(new Set(issues)),
    };
};

export const analyzeGeneratedAppstoreMetadata = (payload) => {
    const subtitleAnalysis = analyzeGeneratedSubtitleOptions(payload?.subtitleOptions);
    const keywordsAnalysis = analyzeGeneratedKeywords(payload?.keywords);

    return {
        subtitleOptions: subtitleAnalysis.options,
        keywords: keywordsAnalysis.keywords,
        issues: Array.from(new Set([...subtitleAnalysis.issues, ...keywordsAnalysis.issues])),
    };
};

export const analyzeGeneratedDescription = (rawValue) => {
    const text = sanitizeDescription(rawValue);
    const blocks = splitBlocks(text);
    const nonEmptyLines = text
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
    const bulletLines = nonEmptyLines.filter((line) => BULLET_LINE_PATTERN.test(line));
    const bulletBlockIndexes = blocks.flatMap((block, index) => {
        const blockLines = block
            .split(/\r?\n/)
            .map((line) => line.trim())
            .filter(Boolean);
        if (blockLines.length > 0 && blockLines.every((line) => BULLET_LINE_PATTERN.test(line))) {
            return [index];
        }
        return [];
    });
    const issues = [];

    if (!text) issues.push('empty');
    if (text.length < MIN_DESCRIPTION_LENGTH) issues.push('too_short');
    if (blocks.length < 2) issues.push('too_few_paragraphs');
    if (blocks.length > 4) issues.push('too_many_paragraphs');

    if (bulletLines.length > 0) {
        const bulletRatio = bulletLines.length / Math.max(nonEmptyLines.length, 1);
        const hasSingleMiddleBulletBlock =
            bulletBlockIndexes.length === 1 &&
            bulletBlockIndexes[0] > 0 &&
            bulletBlockIndexes[0] < blocks.length - 1;
        const tooManyBullets = bulletLines.length > 4 || bulletRatio > 0.35;
        if (!hasSingleMiddleBulletBlock || bulletBlockIndexes.length > 1 || tooManyBullets) {
            issues.push('bullet_heavy');
        }
    }

    const closingBlock = blocks[blocks.length - 1] || '';
    if (GENERIC_CTA_PATTERNS.some((rx) => rx.test(closingBlock))) {
        issues.push('generic_cta');
    }

    if (hasInternalLeakSignals(text)) {
        issues.push('internal_leak');
    }

    return {
        text,
        issues: Array.from(new Set(issues)),
        metrics: {
            charCount: text.length,
            paragraphCount: blocks.length,
            bulletLineCount: bulletLines.length,
            bulletBlockCount: bulletBlockIndexes.length,
        },
    };
};
